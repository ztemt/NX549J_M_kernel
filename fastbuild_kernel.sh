...
INIT_TARGET_PRODUCT=NX549J
FINAL_TARGET_PRODUCT=NX549J
...
[0m[01;31mUsage:[0m
	1. ./zte_build/fastbuild_kernel.sh NX543J
	1. ./zte_build/fastbuild_kernel.sh NX543J perf
	2. ./zte_build/fastbuild_kernel.sh NX549J
	2. ./zte_build/fastbuild_kernel.sh NX549J perf
	3. ./zte_build/fastbuild_kernel.sh NX551J
	3. ./zte_build/fastbuild_kernel.sh NX551J perf
[01;32m===========================================================[0m
...
if [ "${INIT_TARGET_PRODUCT}" == "NX543J" ]
then
	FINAL_TARGET_PRODUCT=NX543
fi

if [ "${INIT_TARGET_PRODUCT}" == "NX549J" ]
then
	FINAL_TARGET_PRODUCT=NX549
fi

if [ "${INIT_TARGET_PRODUCT}" == "NX551J" ]
then
	FINAL_TARGET_PRODUCT=NX551
fi
...
################################################################################################
export ZTEMT_DTS_NAME=msm8953-mtp-${FINAL_TARGET_PRODUCT}
echo -e "\033[01;32mZTEMT_DTS_NAME = ${ZTEMT_DTS_NAME}\033[0m"

if [ "${KERNEL_PERF_DEFCONFIG}" == "perf" ]
then
	KERNEL_DEFCONFIG=msm8953-${FINAL_TARGET_PRODUCT}-perf_defconfig
else
	KERNEL_DEFCONFIG=msm8953-${FINAL_TARGET_PRODUCT}_defconfig
fi
...
make -C kernel O=../out/target/product/${FINAL_TARGET_PRODUCT}/obj/KERNEL_OBJ ARCH=arm64 CROSS_COMPILE=aarch64-linux-android- ${KERNEL_DEFCONFIG}


################################################################################################
echo -e "\033[01;32mBuilding kernel...\033[0m"
make  -C kernel O=../out/target/product/${FINAL_TARGET_PRODUCT}/obj/KERNEL_OBJ ARCH=arm64 CROSS_COMPILE=aarch64-linux-android- KCFLAGS=-mno-android  -j`grep processor /proc/cpuinfo |wc -l`
RET_VAL=$?
make -C kernel O=../out/target/product/${FINAL_TARGET_PRODUCT}/obj/KERNEL_OBJ ARCH=arm64 CROSS_COMPILE=aarch64-linux-android- KCFLAGS=-mno-android modules
make -C kernel O=../out/target/product/${FINAL_TARGET_PRODUCT}/obj/KERNEL_OBJ INSTALL_MOD_PATH=../../system INSTALL_MOD_STRIP=1 ARCH=arm64 CROSS_COMPILE=aarch64-linux-android- modules_install
...
cp -rf out/target/product/${FINAL_TARGET_PRODUCT}/obj/KERNEL_OBJ/arch/arm64/boot/Image.gz-dtb out/target/product/${FINAL_TARGET_PRODUCT}/kernel
...
out/host/linux-x86/bin/mkbootimg  --kernel out/target/product/${FINAL_TARGET_PRODUCT}/kernel --ramdisk out/target/product/${FINAL_TARGET_PRODUCT}/ramdisk.img --cmdline "console=null androidboot.console=ttyHSL0 androidboot.hardware=qcom msm_rtb.filter=0x237 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 androidboot.bootdevice=7824900.sdhci earlycon=msm_hsl_uart,0x78af000" --base 0x80000000 --pagesize 2048  --output out/target/product/${FINAL_TARGET_PRODUCT}/boot.img
