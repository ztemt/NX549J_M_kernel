...
static int wp_charging_enable_cntl(struct weipu_plat *wp_chip, int enable)
{
	...
	if(enable){
		...
		cancel_delayed_work(&wp_chip->wp_chg_work);
	}
...
static void wp_chg_worker(struct work_struct *work)
{
	...
	wpchg_info("->En=%d St=%d Tbat=%d Vbat=%d Ibat=%d Vbus=%d Tusb=%d Vote=0x%x\n",
		wp_chip->en_st,
...
static irqreturn_t weipu_chg_handler(int irq, void *_chip)
{
	...
	if( wp_chip->weipu_chg_st ){
		...
		wp_set_current(wp_chip, WP_DEFAULT_MA);
...
static int wp_parse_dt(struct weipu_plat *wp_chip)
{
	...
	wpchg_info("batt_cool=%d batt_warm=%d cv_soc=%d usb_hot=%d\n",
		wp_chip->batt_cool_temp, wp_chip->batt_warm_temp, wp_chip->batt_cv_soc,wp_chip->usb_hot_temp);
