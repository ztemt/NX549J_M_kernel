...
#define SYNAPTICS_VDD_MIN_UV	2850000
#define SYNAPTICS_VDD_MAX_UV	2850000
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
      ...
      printk(" fhandler->fn_number %02x",fhandler->fn_number);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!bdata->cap_button_map) {
		...
		if(!strncmp(rmi->product_id_string, "NUBIA-JDI", 9)) {
				f1a->button_map[0] = bdata->cap_button_map->map[0];
				f1a->button_map[1] = bdata->cap_button_map->map[2];
				f1a->button_map[2] = bdata->cap_button_map->map[1];
		} else {
		...
		bool get)
{
	...
	const struct synaptics_dsx_board_data *bdata =
      ...
      return 0;     //synaptics 1302 power controled instead of power_gpio
	...
	retval = reg_set_optimum_mode_check(rmi4_data->pwr_reg, 15000);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	retval = reg_set_optimum_mode_check(rmi4_data->bus_reg, 10000);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
//nubia for synaptics pinctrl
	...
	if (retval < 0) {
		...
		goto err_enable_irq;
              ...
              printk(" %s irq = %d ",__func__,rmi4_data->irq);
	if (vir_button_map->nbuttons) {
...
#ifdef NUBIA_TP_NODE_IN_KERNEL
	nubia_tp = rmi4_data;
	rmi4_data->nubia_tp_kobj = kobject_create_and_add("tp", kernel_kobj);

	if (!rmi4_data->nubia_tp_kobj) {
		pr_err("[TP]failed to create and add kobject\n");
		return -ENOMEM;
	}

	for (attr_count = 0; attr_count < ARRAY_SIZE(nubia_tp_attrs); attr_count++) {
		retval = sysfs_create_file(rmi4_data->nubia_tp_kobj, &nubia_tp_attrs[attr_count].attr);
		if (retval < 0) {
...
err_sys_creat:
	for (--attr_count; attr_count >= 0; attr_count--)
		sysfs_remove_file(rmi4_data->nubia_tp_kobj, &nubia_tp_attrs[attr_count].attr);
	nubia_tp = NULL;
#endif
