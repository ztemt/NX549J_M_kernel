	...
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if (!strncmp(nubia_mdss_dsi_ctrl->panel_data.panel_info.panel_name,
			"boe otm1906c 1080p 5p2", 22)) {
		//std
		if(val == 127) {
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->r.r = 0x7A57;
		...
		//warm
			...
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->b.b = 0x8000 -blue * (val - 127);
		}
		//cool
		else if(val < 127) {
			//0x8000 - red * (127 - val) / 127;
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->r.r = 0x7A57 - red * (127 - val);
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->g.g = 0x8000;
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->b.b = 0x8000;
		}
	} else {
		...
		//cool
			...
			//0x8000 - red * (127 - val) / 127;
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->r.r = 0x8000 - red * (127 - val);
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->g.g = 0x8000;
			((struct mdp_pcc_data_v1_7 *)(nubia_mdp_pcc_cfg_adjustable.cfg_payload))->b.b = 0x8000;
		}
	}
