...
static int write_reg(struct aw2013_control_data *led,unsigned char reg,unsigned char data)
{
	...
	for (i=0; i<AW2013_I2C_MAX_LOOP; i++)
	{
		...
		pr_debug("aw2013 : write reg[0x%x] , data[%d]!\n",reg,data);
...
static int led_const_on(struct aw2013_control_data *led)
{
	...
	if(led->outn == LED_MIDDLE_CHANNEL){
	//The value of led->grade_scale_fractions and led->grade_scale_numerator range from 1 to 5.
		if((led->min_grade == CONST_MIN_GRADE) ||((led->min_grade*led->middle_grade_percentage) < 100) ){
			...
			LED_DEBUG("led_const_on,min_grade=%d . \n",led->min_grade*led->middle_grade_percentage/100 + led->grade_scale_offset);
		}
	}
	else{
		if((led->min_grade == CONST_MIN_GRADE) ||((led->min_grade*led->side_grade_percentage) < 100) ){
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, led->min_grade);
		}
		else{
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, 
		...
		struct device_attribute *attr, const char *buf,size_t count)
{
	...
	switch (led->blink_mode) {
		...
		case AW_CONST_OFF:
			...
			break;
		case AW_AUTO_BREATH:
			led->Rise_Fall_time = led->autoblink_fade_time;
			led->Hold_time = led->autoblink_fullon_time;
			led->Off_time = led->autoblink_fulloff_time;
			led_auto_breath(led);
			break;
...

{
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading middle_grade_percentage, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading side_grade_percentage, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading grade_scale-offset, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading blink_max_grade, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading onceblink_fade_time, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading onceblink_fullon_time, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading onceblink_fulloff_time, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading autoblink_fade_time, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading autoblink_fullon_time, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading autoblink_fulloff_time, rc =  %d\n", ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(client->dev.of_node, "max_current",&aw2013_data->max_current);
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading max_current, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	if(ret < 0)	{
		dev_err(&client->dev, "aw2013_i2c_rx_byte_data  read chip id fail!!!\n");	
		return ret;	
	}	
	...
	if(0x33 != buf){			
		...
		dev_err(&client->dev, " the chip id  is 0x33\n");	
	...
	if(ret < 0) {
		dev_err(&client->dev,"aw2013_debug: can not get the isr states\n");
	}
...
static int aw2013_suspend(struct i2c_client *cl, pm_message_t mesg)
{
    ...
    if((aw2013_data->outn == LED_MIDDLE_CHANNEL) &&
	...
	LED_DEBUG("ztemt_aw2013_suspend blink_mode=%d \n",aw2013_data->blink_mode );
	return 0;
};
...
static int aw2013_resume(struct i2c_client *cl)
{
    ...
    if((aw2013_data->outn == LED_MIDDLE_CHANNEL) &&
        ...
        LED_DEBUG("ztemt_aw2013_resume blink_mode=%d \n",aw2013_data->blink_mode );
	return 0;
};
