	...
	if (chip->prx_inf.detected == PROX_FAR) {
		...
		if (chip->prx_inf.raw < chip->params.prox_th_min) {
			...
			if (chip->prox_state == PROX_STATE_NONE) {
                chip->prx_inf.detected = PROX_FAR;
                tmd2725_report_prox(chip);
				if (chip->params.prox_th_max != 255) {
			...
			if (chip->prox_state == PROX_STATE_NONE) {
                chip->prx_inf.detected = PROX_NEAR;
                tmd2725_report_prox(chip);
				chip->params.prox_th_min = 180;
				chip->params.prox_th_max = 255;
				update_thresholds = true;
			}
		}
	}
...
static int tmd2725_offset_calibration(struct tmd2725_chip *chip)
{
	...
	*/
	ams_i2c_modify(chip->client, sh, TMD2725_REG_CALIBCFG,
			...
			(0x04 << TMD2725_SHIFT_BINSRCH_TARGET) |
...
static void tmd2725_init_prox_mode(struct tmd2725_chip *chip, bool first)
{
    ...
    if (chip->prx_inf.prox_raw_debug || first){
	    chip->params.prox_th_min = 100;
	    chip->params.prox_th_max = 100;
    }
...
static int tmd2725_prox_thres_calibrate(struct tmd2725_chip *chip)
{
    ...
    chip->params.prox_near_diff = tmd2725_mean_prox_calc(chip);
    chip->params.prox_far_diff = chip->params.prox_near_diff / 2;
    prox_buf[0] = chip->params.prox_raw;
    prox_buf[1] = chip->params.prox_near_diff;
    SENSOR_LOG_INFO("chip->params.prox_near_diff = %d\n",chip->params.prox_near_diff);
    ret = sensor_write_file(PS_CAL_FILE_PATH, prox_buf, sizeof(prox_buf));
    if (ret < 0) {
        SENSOR_LOG_ERROR("write file fail\n");
        return ret;
    }
    return chip->params.prox_near_diff;
}
/*****************/
/* ABI Functions */
/*****************/
...
static struct device_attribute tmd2725_prox_attrs[] = {
	...
	__ATTR(prox_regs, 0440, tmd2725_prox_regs_show, NULL),
};
...
static irqreturn_t tmd2725_irq(int irq, void *handle)
{
	...
	if(!queue_work(chip->ps_workqueue, &chip->ps_irq_work)) {
        SENSOR_LOG_ERROR("queue work fail\n");
        return IRQ_NONE;
	}
	return IRQ_HANDLED;
}
