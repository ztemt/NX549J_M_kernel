...
static void tmd2725_set_defaults(struct tmd2725_chip *chip)
{
	...
	chip->params.prox_drive = PDRIVE_MA(75);
	chip->params.prox_offset = 0;
	chip->params.prox_pulse_len = PG_PULSE_16US;
	...
	sh[TMD2725_REG_PGCFG0]  = chip->params.prox_pulse_cnt | chip->params.prox_pulse_len;
...
static int tmd2725_resume(struct device *dev)
{
	...
	if (chip->als_enabled)
		schedule_delayed_work(&chip->als_work, chip->als_poll_delay);
