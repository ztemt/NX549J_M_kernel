...
static int tmd2725_als_set_enable(struct tmd2725_chip *chip, u8 enable, bool enable_poll)
{
	...
	if (enable) {
        ...
        if (!chip->pdata->power_always_on) {
			chip->pdata->power_state &= ~POWER_ALS_ON;
            if (chip->als_enabled && chip->pdata->power_state == POWER_OFF)
...
static int tmd2725_max_als_value(struct tmd2725_chip *chip)
{
	int val;

	val = chip->shadow[TMD2725_REG_ATIME];
	if (val > 63)
		val = 0xffff;
	else
		val = ((val * 1024) - 1);
...
static int tmd2725_get_lux(struct tmd2725_chip *chip)
{
	...
	*/
	...
	lux1 = ((chip->als_inf.lux1_ch0_coef * ch0) -
		(chip->als_inf.lux1_ch1_coef * ch1)) / chip->als_inf.cpl;
	lux2 = ((chip->als_inf.lux2_ch0_coef * ch0) -
		(chip->als_inf.lux2_ch1_coef * ch1)) / chip->als_inf.cpl;
	lux = max(lux1, lux2);
	lux = min(TMD2725_MAX_LUX, max(0, lux));
	if (chip->pdata->debug_level) {
		SENSOR_LOG_INFO("lux:%d [ch0:%d, ch1:%d, lux1:%d, lux2:%d]\n",
		    lux, ch0, ch1, lux1, lux2);
        SENSOR_LOG_INFO("d_factor:%d seg[0][ch0_coef:%d ch1_coef:%d] seg[1][ch0_coef:%d ch1_coef:%d]\n",
	...
	if (!chip->als_gain_auto) {
		...
		if (ch0 < 100) {
			...
			tmd2725_flush_als_regs(chip);
		} else if (ch0 > tmd2725_max_als_value(chip)) {
...
static int tmd2725_als_calibrate_work(struct tmd2725_chip *chip, const char *cal_data)
{
	...
	if (chip->als_cal_data.base.lux / chip->als_cal_data.cur.lux > 4) {
	    err = -EINVAL;
				...
				const char *buf, size_t size)
{
	...
	SENSOR_LOG_INFO("config seg paramter is [%d %d][%d %d]\n",
	                chip->params.lux_segment[0].ch0_coef,
	                chip->params.lux_segment[0].ch1_coef,
	                chip->params.lux_segment[1].ch0_coef,
	                chip->params.lux_segment[1].ch1_coef);
