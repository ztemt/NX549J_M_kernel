...
static int apds9922_als_init_client(struct apds9922_als_data *data)
{
	...
	if (err < 0)
		return err;
	data->als_gain_index = APDS9921_DD_ALS_GAIN_3X;	// 3x GAIN
...
static int apds9922_als_enable(struct apds9922_als_data *data, int enable, bool poll_enable)
{
	int err;
	struct apds9922_platform_data *pdata = data->platform_data;

	data->als_enable = enable;
	data->als_poll_enable = poll_enable;
	if (enable) {
	...
	err = apds9922_als_set_enable(data, enable);
	if (err < 0) {
...
static int apds9922_als_calibrate_work(struct apds9922_als_data *data, const char *cal_data)
{
	...
	als_poll_enable = data->als_poll_enable;

	err = apds9922_als_enable(data, true, false);
	if (err < 0) {
...
als_cal_exit:
	...
	apds9922_als_enable(data,als_enable,als_poll_enable);
	return err;
}
                                ...
                                struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if ((val != 0) && (val != 1)) {
     ...
     apds9922_als_enable(data, val, true);
                                        ...
                                        struct device_attribute *attr, const char *buf, size_t count)
{
	...
	apds9922_als_enable(data, true, true);
    return count;
}
                                ...
                                struct device_attribute *attr, char *buf)
{
	...
	{
		apds9922_als_enable(data, true, false);
		msleep(data->als_poll_delay);
		luxValue = apds9922_get_lux_value(data);
		apds9922_calibrate_lux_value(data,luxValue);

		if(false == data->als_poll_enable)
			apds9922_als_enable(data, false, false);
...
{
	struct apds9922_als_data *data = dev_get_drvdata(dev);
	return sprintf(buf, "prev_lux=%d-cal_lux=%d\n", data->als_prev_lux, data->als_cal_lux);
}
...
static struct device_attribute attrs_als_device[] = {
	...
	__ATTR(fac_calibrate, 0444, apds9922_als_factory_calibrate_show, apds9922_als_factory_calibrate_store),
...
//	struct i2c_client *client = pdata->client;
	...
	if(APDS9922_ALS_POLLING && data->als_enable && data->als_poll_enable)
		schedule_delayed_work(&data->als_dwork, msecs_to_jiffies(data->als_poll_delay));
